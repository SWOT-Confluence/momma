...
#
momma <- function(node_id, stage, width, slope, shape.param, Qgage = NA, Qm_prior, Qb_prior = NA, Qmon_prior,
                  Yb_prior = NA, obs_times, invalid_time, known_ezf = NA, known_bkfl_stage = NA,
  ...
  shape_param <- shape.param # channel shape parameter. 2 = parabolic
  # print(paste(">>> shape parameter = ", shape_param, " <<<"))
  # if (shape_param == 2) {print("No shape parameter")}
  ...
  if (nrow(df) >= min_nobs_mean) {
    ...
    cal.Qmean <- calibrate.qmean.prior(dframe = df, Qmean_prior = Qm_prior,
                                       Qmon_prior = Qmon_prior, obs_times = obs_times, invalid_time = invalid_time,
    ...
    if (df$nb[1] == resist.min & zero.stage > zero.stage.floor) {
      ...
      cat("[FINISH] Small Re-Calibration\n")
